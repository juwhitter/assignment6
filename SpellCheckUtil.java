...
public class SpellCheckUtil 
{
  ...
  public List<String> spellCheck(File documentFile) 
  {
	...
	// Continue to check each word until it reaches the end of the file
	for (int i = 0; i < wordsToCheck.size(); i++)
	{
		// If the word is not contained in the dictionary then it will be added to the returned list
		if (!dictionary.contains(wordsToCheck.get(i)))
		{
			mispelledWords.add(wordsToCheck.get(i));
		}
	}
